...
async function testSimpleOptimizedWorkflow() {
    try {
        console.log('🚀 [Test] Test Simple Optimized Workflow - Debug từng bước...');
        ...
        const response = await fetch(`${serverUrl}/api/veo3-complete-workflow`, {
            ...
            body: JSON.stringify({
                ...
                duration: 8,
                frameInterval: 1,
                maxFrames: 2, // Giảm số frames để test nhanh hơn
                outputDir: './temp/test-optimized-8s'
            })
        });
        
        const result = await response.json();
        console.log('🎬 [Step 1] Simple optimized workflow result:', result.success ? '✅ Success' : '❌ Failed');
...
console.log('🚀 [START] Test Simple Optimized Workflow...');
testSimpleOptimizedWorkflow();
