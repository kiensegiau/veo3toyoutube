...
async function mergeMH370Video() {
    try {
        console.log('🎬 [MH370] Ghép video MH370 đã có...');
        ...
        // Tìm tất cả video MH370
        const files = fs.readdirSync(audioDir);
        const mh370Videos = files.filter(file => file.includes('mh370') && file.endsWith('.mp4'));
        ...
        const validVideoFiles = mh370Videos.map(video => {
